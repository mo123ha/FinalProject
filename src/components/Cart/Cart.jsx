...
{cartdetails?.length >0?<div className='bg-gray-50 h-[50x] py-20  my-8  '>
  ...
  <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
    ...
    <tbody>
    <div className=' flex justify-between items-center w-full  my-4 py-4'>
...
<h1 className='font-semibold text-black'>total Price:  <span className='text-[#22DB14]'>{cartdetails?.totalCartPrice}</span></h1>
<h1  className='font-semibold text-black'>total number of items: <span className='text-[#22DB14]'> {countItems}</span></h1>
  </div>

    {cartdetails?.products?.map((product)=> <tr key={product.product._id} className=   " flex justify-between items-center    bg-gray-50 rounded-lg border-b dark:bg-gray-800 dark:border-gray-700  dark:hover:bg-gray-600">
        <td className="p-4  flex items-center">
          <img src={product.product.imageCover} className="w-16 md:w-32 max-w-full max-h-full" alt="Apple Watch" />
          <td className="px-6 py-4 font-semibold text-black dark:text-white">
            <p className='text-lg font-semibold'>{product.product.title}</p>
       <p className='p-2'>    {product.price}</p>
       <span className=" cursor-pointer     font-medium text-red-600 dark:text-red-500 hover:underline   " onClick={()=>{
